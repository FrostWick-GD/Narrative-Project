...
public class ItemInspect2D : MonoBehaviour, IInteractable
{
    [System.Serializable]
    ...
    public void Interact(FPC_CustomAction fpc_ca)
    {
        if (!isCurrentlyInteractedWith)
        {
            ...
            fpc_ca.PlayerSwitchToInspect2DMode();
            UI_Manager.Instance.GetUI_Player_Inspect2D().InitializeInspect2DView(sprites, statusList[currentStatus].objectName, description);
        ...
        else if (isCurrentlyInteractedWith)
        {
